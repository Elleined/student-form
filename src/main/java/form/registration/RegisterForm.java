...
public class RegisterForm implements UsernameValidator {
    ...
    private boolean checkPassEmpty(@NotNull String pass) {
        return pass.isEmpty();
    }

    private boolean checkPassSpace(@NotNull String pass) {
    ...
    private void insert() {
        try (Connection conn = SQLiteConnection.loginDbConnection()){
    ...
    private void showPassword(@NotNull ItemEvent cbSeePassChecked) {
    ...
    private char seePassword(@NotNull JCheckBox checkBox) {
        return checkBox.isSelected() ? 0 : '*';
    }

    private @NotNull Icon selectIcon(@NotNull JCheckBox checkBox) {
